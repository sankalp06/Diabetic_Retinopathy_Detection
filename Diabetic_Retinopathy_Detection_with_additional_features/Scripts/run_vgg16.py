...
from models.vgg16_model import create_vgg16_model, vgg16_preprocess_input
...
scaler_file = 'models_saved/additional_features_scaler_vgg16.pkl'
save_scaler(scaler, scaler_file)

# Load and preprocess images
images = load_images(df['full_path'], preprocess_func=lambda x: load_and_preprocess_image(
    x, target_size=(224, 224), preprocessing_function=vgg16_preprocess_input))

# Split the data into training and testing sets
X_train_img, X_test_img, X_train_feat, X_test_feat, y_train, y_test = split_data(images, additional_features, labels)

# Create the VGG16 model
custom_layers_vgg16 = [Dense(256, activation='relu'), Dropout(0.5), Dense(128, activation='relu')]
vgg16_input, vgg16_output = create_vgg16_model(custom_layers=custom_layers_vgg16)
...
# Combine the VGG16 model with additional features
additional_layers = [Dense(128, activation='relu'), Dropout(0.5)]
model_vgg16 = create_combined_model(vgg16_input, vgg16_output, additional_input, additional_layers=additional_layers)
...
history_vgg16 = compile_and_train_model(
    model_vgg16, X_train_img, X_train_feat, y_train, X_test_img, X_test_feat, y_test, callbacks=custom_callbacks
)
...
evaluate_and_save_model(model_vgg16, X_test_img, X_test_feat, y_test, model_vgg16_save_path, metrics_save_path)
